...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("Fabi")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`Fabi | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`Fabi | ${route.data.id}`)
    }
  })
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "docs.open",
      onSelect: () => {
        open("https://github.com/Noagiannone03/fabi").catch(() => {})
        dialog.clear()
      },
      category: "System",
    },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Fabi v${result.data.version}. Please restart the application.`,
    )
